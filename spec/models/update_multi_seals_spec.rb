...
RSpec.describe UpdateMultiSeals, type: :model do
  ...
  describe "numbers_sealsid_list" do
    ['', [""]].each do |value|
      context "with #{value}" do
        it 'is invalid.' do
          multi_seals.numbers_sealsid_list = value
          expect(multi_seals).not_to be_valid
          expect(multi_seals.errors[:numbers_sealsid_list]).to be_present
        end
      end
    end
  end

  describe ".updateSelectedSeals", focus: true do
    before{
    ...
    context "with empty numbers_sealsid_list" do
      it 'returns false.' do
        multi_seals.numbers_sealsid_list = ''
        expect(multi_seals.updateSelectedSeals).to eq(false)
      end
    end

    context "with existed seal ids in the list" do
      let(:selected_seals){ Seal.find(multi_seals.numbers_sealsid_list) }
      ...
      it 'sets sealed_date of selected seals to 2016-03-31.' do
        selected_seals.each do |seal|
          expect(seal.sealed_date).to eq(Date.parse(multi_seals.sealed_date))
        end
      end
      ...
      it 'keeps sealed_flag of NOT selected seals as false.' do
        unselected_seals = Seal.where.not(
          id: multi_seals.numbers_sealsid_list
        )
    ...
    context "with NOT existed seal ids in the list" do
      ...
      before{
        multi_seals.numbers_sealsid_list = [ 1, 3, 12, 5, 6]
        multi_seals.updateSelectedSeals
      }

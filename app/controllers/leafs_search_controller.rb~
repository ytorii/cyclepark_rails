  ...
  def index
    ...
    respond_to do |format|
      if ransack_validator.invalid?
        format.html { redirect_to menu_path, alert: ransack_validator.errors.full_messages }
        format.json { render json: @ransack_validator.errors, status: :unprocessable_entity }
      elsif @leafs.size == 0
        format.html { redirect_to menu_path, alert: ['指定したリーフは存在しません。']  }
